...
from lumix.linen import InformationEncoder, UnitaryLinear


ARTIFACT_DIR = Path("artifacts/mnist_nonlinearity_comparison")
CACHE_PATH = ARTIFACT_DIR / "cache" / "mnist_fourier_magnitude_radius2.npz"
...
ModelKind = Literal["linear", "repeated_phase", "williamson"]
...
def crop_spectrum(values: np.ndarray, radius: int) -> np.ndarray:
    center = values.shape[-1] // 2
    return values[:, center - radius : center + radius, center - radius : center + radius]


def fourier_magnitude_features(images: np.ndarray) -> np.ndarray:
    spectrum = np.fft.fftshift(np.fft.fft2(images.astype(np.float32) / 255.0), axes=(1, 2))
    magnitudes = np.log1p(np.abs(crop_spectrum(spectrum, RADIUS))).reshape(images.shape[0], -1)
    norm = np.linalg.norm(magnitudes, axis=-1, keepdims=True)
...
def load_or_create_cache() -> DataSplit:
    ...
    x_train = fourier_magnitude_features(raw["x_train"])
    x_test = fourier_magnitude_features(raw["x_test"])
...
class MNISTOpticalModel(nn.Module):
    ...
    def __call__(self, features: jnp.ndarray) -> jnp.ndarray:
        ...
        encoded = encoder(self.phase_scale * features)
        ...
        if self.kind == "repeated_phase":
            for layer_index in range(self.layers):
                fields = self._unitary(layer_index)(fields * encoded)
            return class_probs(intensity(fields), NUM_CLASSES)
...
def write_report(results: list[dict[str, Any]]) -> None:
    ...
    lines = [
        "# MNIST Nonlinearity Comparison",
        "",
        f"Timestamp: {datetime.now().replace(microsecond=0).isoformat()}",
        "",
        "Dataset: MNIST, transformed to 16 phase-only inputs by taking a 4x4 low-frequency Fourier magnitude crop, log-scaling it, and L2-normalizing each vector.",
        "",
        f"Training: {EPOCHS} epochs, batch size {BATCH_SIZE}, Adam lr={LEARNING_RATE}. Output is normalized class intensity with cross-entropy for every model.",
        "",
        f"Williamson uses alpha/tap={WILLIAMSON_TAP}, initial g_phi={WILLIAMSON_GAIN / float(np.pi):.2f}pi, and alpha is fixed.",
        "",
        "| Model | Kind | Layers | g_phi/pi | phi_b/pi | Train g_phi | Train phi_b | Learnable Optical Params | Stored Optical Params | Train Acc | Val Acc | Val Loss |",
        "|---|---|---:|---:|---:|---|---|---:|---:|---:|---:|---:|",
    ]
    for result in results:
        if result["williamson"] is None:
            gain = ""
            bias = ""
            train_gain = False
            train_bias = False
        else:
            gain = f"{result['williamson']['gain'] / float(np.pi):.2f}"
            bias = f"{result['williamson']['bias'] / float(np.pi):.2f}"
            train_gain = bool(result["williamson"]["train_gain"])
            train_bias = bool(result["williamson"]["train_bias"])
        lines.append(
            f"| {result['name']} | {result['kind']} | {result['layers']} | {gain} | {bias} | "
...
def main() -> None:
    ...
    configs = [
        ...
        Config("linear-depth4", "linear", 4),
        Config("repeated-phase-depth3", "repeated_phase", 3),
        Config("repeated-phase-depth4", "repeated_phase", 4),
        Config("williamson-depth3-bias050-fixed", "williamson", 3, williamson_bias=0.50 * float(np.pi)),
        Config("williamson-depth4-bias050-fixed", "williamson", 4, williamson_bias=0.50 * float(np.pi)),
        Config(
