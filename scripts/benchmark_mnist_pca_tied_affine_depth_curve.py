...
RESULTS_PATH = ARTIFACT_DIR / "tied_affine_depth_curve_trainable_softmax_results.json"
SUMMARY_PATH = ARTIFACT_DIR / "tied_affine_depth_curve_trainable_softmax_summary.md"
...
Variant = Literal["tied_unitary_affine_repeated", "tied_subunitary_affine_repeated", "tied_williamson"]
...
class TiedAffineOpticalModel(nn.Module):
    variant: Variant
    layers: int
    phase_scale: float
    fixed_logit_scale: float = FIXED_LOGIT_SCALE
    ...
    def _phase_affine_params(self) -> tuple[jnp.ndarray, jnp.ndarray]:
        scales = self.param(
            "phase_scale",
            lambda key: jnp.full((self.layers,), self.phase_scale, dtype=jnp.float32),
        )
        offsets = self.param(
            "phase_offset",
            lambda key: jnp.zeros((self.layers, WIDTH), dtype=jnp.float32),
        )
    ...
    def __call__(self, features: jnp.ndarray) -> tuple[jnp.ndarray, jnp.ndarray, jnp.ndarray]:
        ...
        if self.variant == "tied_unitary_affine_repeated":
            unitary = UnitaryLinear(width=WIDTH, name="shared_unitary")
            scales, offsets = self._phase_affine_params()
            for layer_index in range(self.layers):
                phase_mask = encoder(scales[layer_index] * features + offsets[layer_index])
                fields = unitary(fields * phase_mask)

        elif self.variant == "tied_subunitary_affine_repeated":
...
def run_config(config: Config, data: PCA.DataSplit) -> dict[str, Any]:
...
LABELS = {
    "tied_unitary_affine_repeated": "Tied unitary affine repeated",
    "tied_subunitary_affine_repeated": "Tied subunitary affine repeated",
...
def write_report(results: list[dict[str, Any]]) -> None:
    ...
    lines = [
        "# MNIST PCA Tied Affine Re-Encoding Depth Curve",
        "",
        ...
        "",
        "Tied affine repeated variants reuse one shared optical device at every layer, but each re-encoding pass learns one scalar data phase scale and one 16-channel phase offset.",
        "",
        "Tied Williamson is kept as the no-repeated-encoding reference: one input phase encoding, one shared unitary, and one shared trainable Williamson gain/bias reused across layers.",
        "",
...
def main() -> None:
    ...
    for depth in range(1, 6):
        configs.extend(
            [
                Config(f"tied-unitary-affine-repeated-depth{depth}", "tied_unitary_affine_repeated", depth),
                Config(f"tied-subunitary-affine-repeated-depth{depth}", "tied_subunitary_affine_repeated", depth),
                Config(f"tied-williamson-depth{depth}", "tied_williamson", depth),
            ]
        )
